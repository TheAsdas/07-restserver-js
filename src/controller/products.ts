...
export const modify = (): RequestHandler => async (req, res) => {
	try {
		...
		if (
			category &&
			!(await Category.exists({ _id: category as string, state: true }))
		)
			throw RequestError(INVALID_REFERENCE);
		if (name && (await Product.exists({ name })))
			throw RequestError(ALREADY_EXISTS);

		data.editedBy = (req as any).user._id;
		data.category = category;

		const product = await Product.findByIdAndUpdate(id, data).populate(
			productPopulateParams
		);
